...
class 规则执行器:
    ...
    def 每根K线处理(self, K线数据, 当前索引):
        ...
        # 收盘后为下一根预挂单准备哨兵价。当前模式本根成交使用的
        # 仍是开盘前已有的哨兵价；这里生成的新价格不会再次检查本根。
        self._计算哨兵价(K线数据, 当前索引)
        # 实验开关：允许新哨兵价在形成当根参与成交检查。
        # 默认关闭，避免改变正式策略的“下一根执行”时序。
        if (self.哨兵价本根新形成
                and self._核心模块启用('本根形成立即成交', False)
                and 允许旧版本根回填(self.买入时机模式)
                and not 待买入已成交):
        ...
        for 持仓 in self.当前持仓.values():
            ...
            if 当前最高 is not None and not pd.isna(当前最高):
                ...
                # 网格加仓：更新“上次加仓后最高价基准”（仅用于下一根，避免同根未来函数）
                if '网格_基准最高价' in 持仓:
                    if 持仓.get('网格_最近加仓索引') == 当前索引:
                        # 本根发生加仓时，不把本根最高价计入“加仓后高点”，避免引入同根顺序假设
                        pass
                    else:
                        持仓['网格_基准最高价'] = max(float(持仓.get('网格_基准最高价', 0) or 0), float(当前最高))
    ...
    def _预计算本根哨兵价(self):
        ...
        if self.哨兵价已形成 and self.哨兵价当前 is not None:
            return
    ...
    def _检查买入(self, K线数据, 当前索引):
        ...
        if (该股票代码 in self.当前持仓
            ...
            return
        if 该股票代码 in self._本根已买入股票:
            self.本根决策["动作原因"] = "同一根K线已经买入，禁止重复普通买入"
            return
        
        # 过滤：哨兵价形成类型必须在已启用的买入规则中
        if not any(规则['类型'] == self.哨兵价形成类型 for 规则 in self.买入规则列表):
        ...
        # 无论哨兵价是否刚形成，都必须由当前K线价格突破后才能成交。
        if not 价格已突破:
            self.本根决策["动作原因"] = (
                "已有哨兵价，但本根收盘价尚未确认突破"
                if 使用收盘确认(买入时机模式)
                else "已有哨兵价，但本根最高价尚未突破"
            )
            return  # 价格未突破哨兵价
    ...
    def _执行买入(self, 规则, K线数据, 当前索引, 当前RSI, 哨兵价=None,
        ...
        最低结构性金额 = 100.0 if self.运行参数.get('多股资金池模式', False) else 10000.0
        if 结构性可用金额 < 最低结构性金额:
        ...
        if 指定股数 is not None:
            try:
                ...
                股数 = 0
            股数 = max(100, int(股数 / 100) * 100)
        else:
            股数 = max(100, int(买入金额上限 / 买入价 / 100) * 100)
        ...
        # 网格字段：在加仓成交时推进次数，并将“上次加仓后高点基准”重置为本次成交价（避免同根未来函数）
            原持仓['网格_已加仓次数'] = int(原持仓.get('网格_已加仓次数', 0) or 0) + 1
            原持仓['网格_最近加仓索引'] = 当前索引
            原持仓['网格_基准最高价'] = float(前复权成交价)
        else:
    ...
    def _计算哨兵价(self, K线数据, 当前索引):
        ...
        # 同一层信号不重复重算；出现新的阈值上穿时允许重新形成哨兵价。
        # 新信号形成的哨兵价可以低于旧哨兵价，这是一次新的信号归属，
        # 但没有新信号时旧哨兵价仍只允许向上跟踪。
        _新阈值信号 = bool(
            _上穿阈值 is not None
    ...
    def _检查加仓(self, 持仓, K线数据, 当前索引, 股票_code=None):
        ...
        # 网格触发先累计，不要求本根立即有买入信号；只有成交时才重新确认买入条件。
        ...
        for 触发 in 触发列表:
            ...
            if not 仅执行待单:
                ...
                try:
                    持仓['网格_基准最高价'] = float(K线数据.get('前复权_最高') or 持仓.get('网格_基准最高价'))
                except (TypeError, ValueError):
                    pass
            ...
            if not 本根有新信号:
                self.本根决策.setdefault('决策记录', {}).setdefault('买入', {}).update({
                    '加仓检查': '待成交',
                    '加仓拦截原因': '网格已触发，但本根没有新的RSI上穿买入信号',
                    '待加仓层级': ' + '.join(f'L{x}' for x in 持仓['网格_待加仓层级']),
                    '待加仓股数': 持仓['网格_待加仓股数'],
                })
                continue

            新规则 = self._获取买入规则(本根新信号)
            if not 新规则:
                continue
            ...
            成交哨兵价 = self.哨兵价当前 if 本根有新信号 and self.哨兵价当前 else 触发价
            已加仓 = self._执行买入(
                规则=None, K线数据=K线数据, 当前索引=当前索引,
                当前RSI=当前RSI,
                哨兵价=成交哨兵价,
