...
def 运行方案(source, output, stocks, start, end, workers, disable_rsi70,
          resonance, minimum_count=2):
    复制配置(source, output)
    if disable_rsi70:
        设置关闭_rsi70(output)
    if resonance:
        设置共振(output, minimum_count)
    return 回测(output, stocks, start, end, workers)


def main():
    parser = argparse.ArgumentParser(description="RSI70与共振过滤组合Walk-forward验证")
    ...
    output = args.output_dir or os.path.join(
        项目根目录, "10_实验记录", f"RSI70共振WalkForward_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    )
    ...
    plans = [
        ("完整基线", False, False),
        ("关闭RSI70", True, False),
        ("关闭RSI70_开启宽松共振", True, True),
    ]
    for index, window in enumerate(windows, 1):
        start, end = window["验证期"]
        window_results = {}
        for name, disable_rsi70, resonance in plans:
            config_dir = os.path.join(output, f"窗口{index}", name)
            window_results[name] = 运行方案(
                source, config_dir, args.stocks, start, end, args.workers,
                disable_rsi70, resonance, args.minimum_count,
            )
    ...
    payload = {
        "方案": "关闭RSI70 + RSI多信号宽松共振", "股票池": args.stocks,
        "参数": {"共振最少通过数量": args.minimum_count, "最低量比": 1.0}, "结果": results,
    }
