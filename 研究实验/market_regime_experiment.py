...
def 设置过滤(config_dir, enabled):
    path = os.path.join(config_dir, "过滤因子配置.yaml")
    with open(path, encoding="utf-8") as source:
        config = yaml.safe_load(source)
    item = next(row for row in config["过滤因子列表"] if row["英文标识"] == "market_regime_filter")
    item["启用"] = enabled
    item["防守模式允许信号"] = ["RSI上穿20"]
    item["收缩模式允许信号"] = ["RSI上穿20"]
...
def main():
    parser = argparse.ArgumentParser(description="市场状态过滤训练期实验")
    ...
    output_dir = os.path.join(
        项目根目录, "10_实验记录", f"市场状态训练实验_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    )
    os.makedirs(output_dir, exist_ok=True)
    results = {}
    for name, enabled in [("关闭过滤", False), ("启用市场状态过滤", True)]:
        config_dir = os.path.join(output_dir, name)
        复制配置(os.path.abspath(args.source_config), config_dir)
        设置过滤(config_dir, enabled)
