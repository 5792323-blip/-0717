               ...
               # ─── 阶梯式哨兵价 ───
                ...
                for _字段 in ['哨兵价', '哨兵价前值', '本根触发哨兵价', '最终买入触发价当前']:
                    try:
                        ...
                        d[_字段] = None
                # 反推价及其信号也必须使用执行器真实状态；报告层重算值只
                # 作为没有持仓过程字段的旧结果兼容兜底。
                if '本根反推价' in _真实状态:
                    d['反推价'] = _真实状态.get('本根反推价')
                    d['反推价_前复权'] = _真实状态.get('本根反推价')
                if '本根反推信号类型' in _真实状态:
                    d['反推信号'] = _真实状态.get('本根反推信号类型')
                if '本根反推目标RSI' in _真实状态:
                    d['反推目标RSI'] = _真实状态.get('本根反推目标RSI')
