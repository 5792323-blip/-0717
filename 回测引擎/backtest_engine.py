...
def 跑回测(股票代码="600519", 开始日期=None, 结束日期=None,
    ...
    # 每次实验按配置重算三种RSI。必须在日期截取之前计算，
    # 否则回测起点前的历史会丢失，导致前若干根K线的RSI失真。
    配置目录 = 配置目录 or os.path.join(项目根目录, '1_策略配置')
    配置目录 = os.path.abspath(配置目录)
    with open(os.path.join(配置目录, '参数配置.yaml'), encoding='utf-8') as 配置文件:
        _参数配置 = yaml.safe_load(配置文件) or {}
    _技术参数 = _参数配置.get('技术指标参数', {})
    from 策略引擎.rsi_source import 准备RSI指标
    数据 = 准备RSI指标(
        数据,
        价格源=_技术参数.get('RSI价格源', 'close'),
        RSI周期=_技术参数.get('RSI周期', 14),
        均线周期=_技术参数.get('RSI均线周期', 20),
    )
    # ATR周期也必须跟随本次实验配置重算，不能继续复用数据文件里的固定ATR_14。
    from 策略引擎.atr import 计算ATR
    数据 = 数据.copy()
    数据['ATR_14'] = 计算ATR(
        数据['前复权_最高'], 数据['前复权_最低'], 数据['前复权_收盘'],
        周期=int(_技术参数.get('ATR周期', 14)),
    )
    ...
    if len(数据) < 100:
        输出(f"❌ 数据不足 (仅{len(数据)}行)")
        return None

    # 盘中不能预知当日完整成交额，因此用上一交易日总成交额控制流动性。
    if '成交额' in 数据.columns:
        ...
        数据['_上一交易日成交额比20日均值'] = 日期键.map(上一日成交额 / 上一日成交额均值20)
