...
证据：截至稳定 HEAD `ce122a94b1601396fd36273a0994be2b7a052f52`，该编号只存在于本状态文件；全量 Git 历史、分支、tag、代码和测试无直接定义。

已排除的候选：报表旧结果回退/重算、旧工作台字段迁移、旧回放脚本、旧执行器适配器、成交数量字段别名。它们均缺少与 COMPAT-P2-019 的唯一映射。
