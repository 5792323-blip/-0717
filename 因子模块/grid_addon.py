...
class 网格加仓(因子基类):
    def __init__(self, 参数=None):
        ...
        self.加仓模式 = str(self.参数.get("加仓模式", "linear"))
    ...
    def 加仓前检查(self, 持仓, K线数据, 全局状态) -> dict:
        ...
        if self.启用风控限制:
            ...
            if (self.最小加仓间隔K线 > 0
                    and 当前索引 is not None and 最近加仓索引 is not None
            ...
            if (self.禁止加仓持仓K线数 > 0
                    and 当前索引 is not None and 买入时间 is not None
        ...
        # 网格回撤以首次实际买入价为固定锚点，不随持仓后的价格创新高上移。
        # 旧持仓没有该字段时回退到首次买入价，兼容历史回测状态。
        基准买入价 = 持仓.get("网格_首次买入价", 持仓.get("买入价"))
        try:
            基准买入价 = float(基准买入价) if 基准买入价 is not None else None
        except (TypeError, ValueError):
            基准买入价 = None
        if not 基准买入价 or 基准买入价 <= 0:
            return {"触发加仓": False, "说明": "缺少网格首次买入价"}
        ...
        return {
            ...
            "说明": f"模式={self.加仓模式}, 首次买入价={基准买入价:.2f}, 触发价={触发价:.2f}, 最低={当前最低:.2f}, 买入={目标股数}股",
        }
